...
.. glossary::
	...
	DN
		...
		represents the "path to the root" of a node in the :term:`DIT`. E.g.,
	...
	RDN
		...
		the :term:`DN` then ``myfile.txt`` would be the RDN.
	...
	DC
		Domain Component. Usually the two last parts of a :term:`DN`. E.g.,
		``dc=example,dc=com``.
	...
	LDIF
		...
		for representing :term`LDAP` directory content and update requests. LDIF
	...
	SPNEGO
		...
		"spen-go", is a :term:`GSSAPI` pseudo-mechanism used by client-server
	...
	Principal
		...
		user-principals
			:term:`Kerberos` representation of people sitting at a machine.
			Example: ``einstein@EXAMPLE.COM``.

		service-principals
			E.g., :term:`Kerberos` representation of a web server. Example:
