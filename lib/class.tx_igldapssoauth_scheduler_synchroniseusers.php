		...
		if(is_array($uidArray)) {
			foreach($uidArray as $uid) {
				...
				if (tx_igldapssoauth_ldap::connect(tx_igldapssoauth_config::get_values('ldap'))) {
					...
					$search = Tx_IgLdapSsoAuth_Utiliy_Ldap::search($this->config['users']['basedn'], str_replace('{USERNAME}', '*', $this->config['users']['filter']), array('dn'));
					global $TYPO3_CONF_VARS;
					$userList = Tx_IgLdapSsoAuth_Utiliy_Ldap::get_entries();
